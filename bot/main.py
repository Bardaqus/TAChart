...
async def chart(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # параметры (мягкие дефолты)
    tol = 0.02; anchors = 2; span = 10; zz = 0.008
    kv = parse_kv(context.args[2:])
    if "tol" in kv:
        try: tol = float(kv["tol"]); except: ...
    if "anchors" in kv:
        try: anchors = int(kv["anchors"]); except: ...
    if "span" in kv:
        try: span = int(kv["span"]); except: ...
    if "zz" in kv:
        try: zz = float(kv["zz"]); except: ...
    ...
    # ручные линии для этого чата/пары/ТФ
    ...
    try:
        fig, info = render_chart_with_lines(
            df,
            max_lines=7,
            tolerance=tol, min_anchors=anchors, min_span=span, zz_dev=zz,
            manual_lines=manual
        )
...
async def addline_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Загружаем данные и «притягиваем» к экстремумам
    limit = LOOKBACK.get(timeframe, 1200)
    try:
        df = await fetch_ohlcv_df(symbol, timeframe, limit)
        if not isinstance(df, pd.DataFrame) or df.empty:
    ...
    ts0 = _pd.to_datetime(kv["from"])
    ts1 = _pd.to_datetime(kv["to"])

    # берём ближайшие индексы
    ...
    add_line(chat_id, symbol, timeframe, {
        "type": t, "i0": i0, "i1": i1, "slope": m, "intercept": b, "color": color
    })
    await update.message.reply_text(f"Сохранил линию: {symbol} {timeframe} [{t}] i0={i0}, i1={i1}, side={side}")
...
async def listlines_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for i, L in enumerate(items, 1):
        msg += f"{i}) {L.get('type')} i0={L.get('i0')} i1={L.get('i1')} color={L.get('color')}\n"
